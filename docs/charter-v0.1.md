...
> 勘误 E17（§23，[ADR-0042](adr/0042-m1-local-authorization-consume-and-closure.md)）：M1-6 交付本机 `{eventId, sequence}` 引用与 SimulatedRuntime 消费；这不是签名启动凭证。编号切片状态不是 M1 检查点。提问通道（#42）、签名/过期/吊销（#53）与宪章 v0.2 合并仍未交付。
...
| v0.1-errata-4 | 2026-09-04 | [ADR-0042](adr/0042-m1-local-authorization-consume-and-closure.md)：E17。M1-6 本机 `{eventId, sequence}` 消费；E4 编号切片状态记录（不是 M1 检查点）；#42、#53 与 v0.2 仍未交付 |
...
| E17 | §14.3 M1-6 | SimulatedRuntime 按 `{eventId, sequence}` 消费本机 `plan.authorization.evaluated` 事实（[ADR-0042](adr/0042-m1-local-authorization-consume-and-closure.md)）。这不是签名启动凭证。编号切片状态不是 M1 检查点（#38）。提问通道（#42）、签名/过期/吊销（#53）、NativeProcessRuntime 等见该 ADR 未交付清单。M1 切片顺序不变。宪章 v0.2 合并仍待 | ADR-0042 |
