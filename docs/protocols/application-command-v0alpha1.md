...
new fact is written. For `research.decision`, and for the first write of
`run.simulate`, `expectedHead` is the EventStore CAS token carried into that
append. A newer head read while the command is in flight is not adopted. A
mismatch fails `stale-head` and does not append that command's events or a
success receipt. Reusing an event id recovers a receipt only when the stored
fact has the same type and the same semantic content; any other occupant of
that id fails `duplicate-event` and writes nothing. Cross-project documents
