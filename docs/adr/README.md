...
| [0063](0063-m3-native-process-runtime-slice-1.md) | M3 slice 1 local restricted native run and SSH onboarding scaffold | Proposed in this slice | Sealed preflight plus local consume before a fixed noop helper; `ssh` validated then refused; onboarding pack stays `pending-live`; no paid cloud or public MVP |
| [0064](0064-m3-native-runtime-slice-2.md) | M3 slice 2 pinned identity, second profile, local Web onboarding | Proposed in this slice | Digest-pinned interpreter/environment with reverified `restricted-v0alpha2`; optional static `ONBOARDING.html` with no script or network; `ssh` still refused; no paid cloud or public MVP |
| [0065](0065-m3-pack-output-hardening.md) | M3 slice 3 onboarding pack output hardening | Proposed in this slice | Symlink/non-directory outputs refused before any write; `STATUS.json` owner-only on POSIX; CLI `--profile` choices pinned to the registry; `ssh` still refused; no paid cloud or public MVP |
...
result. Data snapshot and checkpoint collect is ADR-0057: Hub revision
