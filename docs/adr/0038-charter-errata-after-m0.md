...
| M1-4 | OpenAI-compatible HTTP adapter; default endpoint is a local MLX-LM / llama.cpp server; remote endpoints require `SecretRef` + `read.external_api` + a budget cap; `budget.reserved` / `consumed` / `exceeded` / `released` are the first runtime-enforced caps | `12-SECB` gate 2; E5 |
