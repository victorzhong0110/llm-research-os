...
| Local/remote Workers | Partially trusted execution nodes | M2-0 loopback long poll + HMAC grants bound to `execute.local` + CPU helper without kernel isolation (ADR-0043, TM-043); non-loopback and OCI/runc pending |
...
| TM-009 | Worker spoofing, replay or stale lease executes a task twice | Cost, corruption or data exposure | Loopback long-poll with HMAC worker session and `rg1` grant token bound to project, worker, grant, task, run, attempt, nonce, image, config, and expiry; short leases; nonce consume; idempotent claim/complete; `resumed` poll does not spawn; expired/revoked new results fail-closed; matching terminal results stay idempotent; non-loopback bind refused | Tested in M2-0 (`tests/test_worker_protocol.py`, `tests/test_worker_faults.py`). Non-loopback HTTPS remains ADR-0021 |
...
| TM-012 | AI or user bypasses approval via a low-level adapter | Governance and budget bypass | Pure trusted-kernel gate revalidates ready reports, binds all three digests and evaluates exact capabilities, permissions and requirements; SimulatedRuntime invokes it before writes and then consumes the cited local `{eventId, sequence}` fact; Worker grants MUST cite an authorized `execute.local` evaluation and the plan's execution object; a `simulate` authorization cannot launch a brick | Gate, T0 integration, local consume, and Worker grant/revoke/binding tests; JWT launch credentials remain out of scope |
...
| TM-043 | A swapped brick, config, inputs or runtime runs under an old authorization, or host Python is treated as a kernel sandbox | Unreviewed code execution; false isolation claims | Plan task config is the execution object; grant cites `execute.local` evaluation plus `imageDigest`/`configDigest`; Worker re-checks before spawn; swapped object fails closed without `Popen`; stdout/stderr limits apply during pipe reads; POSIX process groups are reaped. No seccomp, landlock, or network/filesystem jail is claimed | Binding, swapped-brick, cross-task token, output-cap, and child-reap tests in M2-0. Kernel isolation remains out of scope |
