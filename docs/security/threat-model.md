...
| TM-044 | A Worker opens the control-plane SQLite or shared CAS, or loopback HTTP is treated as a cross-machine Worker, or grant tokens leak in logs | Unauthorized fact mutation; false remote proof; credential disclosure (TM-007) | Isolated processes use separate workdirs and CAS; image GET requires the grant's `imageDigest`; loopback HTTPS pins a local CA; credentials are 0600 and redacted; reconnect does not re-run resumed work. Tests must not claim cross-machine verification | Isolate, GET-binding, HTTPS pin, HMAC reload, credential-mode, and two-process tests in ADR-0044 |
