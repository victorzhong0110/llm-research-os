...
| TM-011 | Event history is edited or a projection is treated as fact | False audit and recovery state | SQLite facts reject UPDATE/DELETE/REPLACE; reads verify canonical JSON, digest and indexes; query/replay CLI and in-memory folds are rebuildable consumers; `run_projections` / spec and artifact indexes rebuild from `events` and are discarded when canonical JSON or digest disagrees; high-water checkpoint is revalidated against live sequence, last digest and schema before reuse | Event source, replay fold, RunControl, checkpoint adversarial tests and query-table rebuild tested |
...
| TM-038 | Leading, repeated or sycophantic questions steer the researcher's decision through the help channel; excessive questioning exhausts human attention | Governance capture; false consensus; north-star metric degraded | Planned (M1-1/M1-5): every decision carries a rationale so steering is auditable; overridden dissents stay in the log; the ledger counts questions and rationale length (ADR-0039 D4) so attention cost is visible; mock-provider adversarial corpus in M1-2 | Not implemented. Metric and corpus required before M1 closure |
