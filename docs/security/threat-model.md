...
| TM-053 | `m2 bench` EventStore fill is treated as Worker/RunControl usage, an isolated Worker shares the control CAS, or an OCI skip is treated as a live engine | False performance/usage proof; shared state; false OCI acceptance | `m2 usage` forbids heartbeat fill; isolated Worker uses a private CAS; report cites spec/runtime/image/config/output; OCI skip is labeled; designated Linux CI fails closed (ADR-0052) | `tests/test_m2_usage.py` |
| TM-054 | A GPU experiment sheet, CUDA Dockerfile, or `training plan` argv is treated as a paid run, v3 `--sft_type` is sent to ms-swift 4.5.2, or `verify.sh` pulls CUDA in ordinary CI | False training success; wrong CLI; unapproved spend; CI disk blow-up | Sheet is `gpu: not-run`; argv is `--tuner_type`; verify.sh forbids `--gpus` and defaults to `skipped-no-build`; ¥20 cap must be recorded before boot; Worker does not stop the cloud instance (ADR-0053) | `tests/test_gpu_experiment_sheet.py`, `tests/test_training_backend.py` |
