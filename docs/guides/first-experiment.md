...
stopped. Sandbox timeout or a killed sandbox process is `unknown`, not
`failed`. Disconnect before complete leaves the lease open (not success).
Heartbeats do not grow the event log.
