...
| --- | --- | --- | --- |
| Remote Worker | Loopback HTTPS Worker plus two-host TLS Worker (`worker.wsl.1`, `worker.wsl.gpu.1`) | EventStore seq 1–153; [eventstore-redacted.json](eventstore-redacted.json); [eventstore-redacted-cuda8-9.json](eventstore-redacted-cuda8-9.json); [eventstore-redacted-cuda10.json](eventstore-redacted-cuda10.json); [https-locate.md](https-locate.md) | Not a productized multi-tenant Worker. #38 open. |
...
| Restore | Overlay `grant.wsl.cuda.9` (partial observe); reports split requested vs observed; leftovers excluded. `grant.wsl.cuda.10` was the authorized 10→12 load-hook shot | seq 140–142; [cuda9-report-correction.md](cuda9-report-correction.md); seq 153 failed [cuda10-failure.md](cuda10-failure.md); `tests/test_worker_gpu.py` | optimizer/scheduler/rng **still unverified**. cuda.10 consumed-failed (`args.json` EACCES). Not retried. |
...
| Code identity | cuda.7 overlay `b3da4bd` / e47279d tree; cuda.8 shared venv miss; cuda.9 `PYTHONPATH` overlay; independent venv of `041365a` (no `workers run`); cuda.10 independent venv of `6e60115` with `workers run` | cuda.10 failed before Trainer load; see [cuda10-failure.md](cuda10-failure.md) |
